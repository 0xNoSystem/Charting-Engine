...
export async function fetchCandles(
    ...
    const filtered = sortCandles(dedupeCandles(clampToRange(raw, startTime, endTime)));
...
async function fetchCoinbaseCandles({
    ...
    const url = baseUrl + buildQuery({ granularity: interval, start: startIso, end: endIso });
...
async function fetchKrakenFuturesCandles({
    ...
    const json = (await res.json()) as {
        result?: { candles?: Array<{ time: number; open: number; high: number; low: number; close: number; volume: number }> };
    };
...
async function fetchHtxCandles({
    ...
    const json = (await res.json()) as {
        data?: Array<{ id: number; open: number; high: number; low: number; close: number; vol?: number; amount?: number; count?: number }>;
    };
    const data = json.data ?? [];
    return data.map((k) => {
...
async function fetchMexcCandles({
    ...
    const startSec = Math.floor(startTime / 1000);
    const endSec = Math.floor(endTime / 1000);
    const url = baseUrl + buildQuery({ interval, start: startSec, end: endSec });
